...
M_LEFT = BP.PORT_C
dps = 300
TURN_VALUE = 1080 #  set this value 
TURN_VALUE2 = 220 #  set this value
try:
    try:
    ...
    for index in range(0,4):
        ...
        while (BP.get_motor_encoder(M_LEFT)<TURN_VALUE):
            pass
        print(f"Turning with encoder value: {BP.get_motor_encoder(M_LEFT)} {BP.get_motor_encoder(M_RIGHT)}")
        BP.offset_motor_encoder(BP.PORT_B, BP.get_motor_encoder(BP.PORT_B)) # reset encoder B
        BP.offset_motor_encoder(BP.PORT_C, BP.get_motor_encoder(BP.PORT_C)) # reset encoder C
        # Turn function
        BP.set_motor_dps(M_LEFT,-103)
        BP.set_motor_dps(M_RIGHT,100)
        while (BP.get_motor_encoder(M_RIGHT)<TURN_VALUE2):
            pass
        print(f"Turn end")
        print(f"Turning encoder :{BP.get_motor_encoder(M_LEFT)} {BP.get_motor_encoder(M_RIGHT)}")
        BP.offset_motor_encoder(BP.PORT_B, BP.get_motor_encoder(BP.PORT_B)) # reset encoder B
        BP.offset_motor_encoder(BP.PORT_C, BP.get_motor_encoder(BP.PORT_C)) # reset encoder C
